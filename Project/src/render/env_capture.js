...
export class EnvironmentCapture {
  ...
  constructor(regl, resourceManager) {
    ...
    const faces = [0, 1, 2, 3, 4, 5].map((side_idx) =>
      this.resourceManager.getTexture(`cube_side_${side_idx}.png`)
    );
  ...
  init_visualization(regl) {
    this.flattened_cubemap_pipeline = regl({
      ...
      vert: this.resourceManager.getShader("cubemap_visualization.vert.glsl"),
      frag: this.resourceManager.getShader("cubemap_visualization.frag.glsl"),
    });
  }
   ...
   */
  capture_scene_cubemap(sceneState, capture_center, scene_renderer_func) {
    ...
    for (let side_idx = 0; side_idx < 6; side_idx++) {
      ...
      cube_camera.mat.view = this.cube_camera_view(
        side_idx,
        capture_center,
        scene_camera.mat.view
      );
      // compute objects transformation matrices for this new camera
      cube_camera.computeObjectsTransformationMatrices(scene.objects);
      ...
      this.run_with_output_framebuffer(
        ...
        () => {
          this.regl.clear({
            color: sceneState.backgroundColor, // reset the canvas color to the same background color defined for the scene in main.js
            depth: 1,
          });

          scene_renderer_func(sceneState);
        }
      );
    }
