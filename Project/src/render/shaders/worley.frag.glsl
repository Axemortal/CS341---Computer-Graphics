...
float worley(vec2 p) {
    ...
    for(int xo = -1; xo <= 1; ++xo) {
        for(int yo = -1; yo <= 1; ++yo) {
            ...
            d = min(d, length2(p - (tp + vec2(w_noise(tp), w_noise(tp + 1.0)) * sin(u_time) * 1.5)));
        }
    }
...
vec3 full_worley(vec2 point) {
    ...
    vec2 glitchUV = uv;
    glitchUV.y += sin(glitchUV.x * 20.0 + u_time) * 0.05;
    glitchUV.x += sin(glitchUV.y * 10.0 + u_time) * 0.03;

    // Glitchy color bands
    float r = sin(glitchUV.x * 8.0 + u_time);
    float g = sin(glitchUV.y * 8.0 + u_time);
