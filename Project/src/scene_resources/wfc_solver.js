...
// Full WFC run: returns a 3D array of final variants
export async function runWFC(dimX = 10, dimY = 10, dimZ = 1) {
    const data = await loadRules();
    // Flatten tile rotations into variants
    const variants = data.tiles.flatMap(tile =>
      tile.rotations.map(rot => ({
        id: rot.id,
        model: tile.model,
    ...
    while (collapseCell(grid)) {
      propagate(grid, compatibility);
    }
  
    // Extract the single chosen variant per cell
