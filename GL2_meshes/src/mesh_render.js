  ...
  render(frame_info, scene_info) {
    ...
    for (const actor of scene_info.actors) {
      ...
      /* #TODO GL2.2.2 
			...
			*/
      // Calculate (VM)^-T for normal transformation
      // First compute VM (model-view matrix)
      const mat_vm = mat4.create();
      mat4.copy(mat_vm, mat_model_view); // mat_model_view is already VM

      // Then compute (VM)^-T
      const mat_vm_inv = mat4.create();
      mat4.invert(mat_vm_inv, mat_vm); // Compute inverse
      mat4.transpose(mat_vm_inv, mat_vm_inv); // Compute transpose

      // Extract 3x3 matrix for normal transformation
      mat3.fromMat4(mat_normals_to_view, mat_vm_inv);
